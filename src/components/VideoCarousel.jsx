...
export default function VideoCarousel(){
    ...
    useEffect(()=>{
        ...
        if(span[videoId]){
            ...
            const animUpdate =()=>{
                anim.progress(videoRef.current[videoid]/higlightsSlides[videoId].videoDuration)
            }
            if(isPlaying){
                    ...
                    <div key={list.id} id='slider' className="sm:pr-20 pr-10">
                        <div className = "video-carousel_container">
                            <div className="w-full h-full flex-center rounded-3xl overflow-hidden bg-black">
                                <video id="video" onLoadedMetadata={(e)=>{handleLoadedMetaData(i,e)}} playsInline={true} preload='auto' muted ref={(el)=>{videoRef.current[i]=el}} onPlay={()=>{
                                    setVideo((prevVideo)=>({
